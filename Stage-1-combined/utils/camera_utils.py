...
def _load_object_mask(object_path, image_name, resolution):
    if not object_path:
        return None
    for ext in (".png", ".jpg", ".npy"):
        candidate = os.path.join(object_path, image_name + ext)
        if os.path.exists(candidate):
            if ext == ".npy":
                seg = torch.from_numpy(np.load(candidate)).long()
                if seg.ndim == 3:
                    seg = seg[..., 0]
                ...
                return seg
            seg_img = Image.open(candidate).resize(resolution, Image.NEAREST)
...
def loadCam(args, id, cam_info, resolution_scale):
    ...
    object_path = getattr(args, 'object_path', '')
    objects = _load_object_mask(object_path, cam_info.image_name, resolution)

    return Camera(colmap_id=cam_info.uid, R=cam_info.R, T=cam_info.T,
                  FoVx=cam_info.FovX, FoVy=cam_info.FovY,
                  image=gt_image, gt_alpha_mask=loaded_mask,
                  image_name=cam_info.image_name, uid=id, data_device=args.data_device,
                  intrinsic_martix=intrinsic, objects=objects)
