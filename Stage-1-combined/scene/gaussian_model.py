...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int,args):
        ...
        self._object_embedding = torch.empty(0)
        self.object_embed_dim = 16
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        object_embedding = torch.randn((fused_point_cloud.shape[0], self.object_embed_dim), device="cuda") * 0.01
        self._object_embedding = nn.Parameter(object_embedding.requires_grad_(True))
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            {'params': [self._object_embedding], 'lr': training_args.feature_lr, "name": "object_embedding"},
        ]
        self.prune_params_names=["xyz","f_intr","opacity","scaling","rotation","object_embedding"]
    ...
    def construct_list_of_attributes(self):
        ...
        for i in range(self._object_embedding.shape[1]):
            l.append('object_emb_{}'.format(i))
        l.append('opacity')
    ...
    def save_ckpt_ply(self, path):
        ...
        object_emb = self._object_embedding.detach().cpu().numpy()
        ...
        attributes = np.concatenate((xyz, normals, f_intr, object_emb, opacities, scale, rotation), axis=1)
    ...
    def load_ckpt_ply(self, path):
        ...
        object_emb_names = [p.name for p in plydata.elements[0].properties if p.name.startswith("object_emb_")]
        object_emb_names = sorted(object_emb_names, key=lambda x: int(x.split('_')[-1]))
        if object_emb_names:
            object_embedding = np.zeros((xyz.shape[0], len(object_emb_names)))
            for idx, attr_name in enumerate(object_emb_names):
                object_embedding[:, idx] = np.asarray(plydata.elements[0][attr_name])
        else:
            object_embedding = np.zeros((xyz.shape[0], self.object_embed_dim), dtype=np.float32)
        ...
        self._object_embedding = nn.Parameter(torch.tensor(object_embedding, dtype=torch.float, device="cuda").requires_grad_(True))
    ...
    def prune_points(self, mask):
        ...
        self._object_embedding = optimizable_tensors["object_embedding"]
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        new_tensor["object_embedding"] = self._object_embedding[selected_pts_mask].repeat(N,1)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        new_tensor["object_embedding"] = self._object_embedding[selected_pts_mask]
