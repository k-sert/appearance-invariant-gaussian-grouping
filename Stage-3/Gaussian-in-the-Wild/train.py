...
from utils.boundary_loss_utils import boundary_identity_contrast_loss, boundary_footprint_loss
...
try:
    ...
    CODECARBON_FOUND = True
except ImportError:
    OfflineEmissionsTracker = None
    CODECARBON_FOUND = False
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, debug_from,args):
    ...
    if getattr(args, "use_codecarbon", False):
        if not CODECARBON_FOUND:
            logging.warning("codecarbon requested but not installed; continuing without emissions tracking.")
        else:
            emissions_dir = os.path.join(dataset.model_path, "codecarbon")
            os.makedirs(emissions_dir, exist_ok=True)
            emissions_tracker = OfflineEmissionsTracker(
                project_name=f"stage3_{dataset_name}",
                output_dir=emissions_dir,
                country_iso_code=getattr(args, "codecarbon_country_iso_code", "NLD"),
                gpu_ids=[0],
            )
            emissions_tracker.start()
            logging.info(f"CodeCarbon tracking started. Output dir: {emissions_dir}")
    ...
    record_loss=[]
    ech_loss=0
    boundary_loss_accum = 0.0
    boundary_footprint_accum = 0.0
    ...
    try:
        ...
        # Render
            ...
            loss_boundary = image.sum() * 0.0
            loss_footprint = image.sum() * 0.0
            if args.use_boundary_loss:
                loss_boundary = boundary_identity_contrast_loss(
                    render_pkg.get("render_identity"), viewpoint_cam,
                    max_points=args.boundary_max_points,
                    k_neighbors=args.boundary_k_neighbors,
                    edge_threshold=args.boundary_edge_threshold,
                    similarity_margin=args.boundary_similarity_margin,
                )
                loss_footprint = boundary_footprint_loss(
                    render_pkg.get("render_identity"), viewpoint_cam,
                    max_points=max(512, args.boundary_max_points // 2),
                    edge_threshold=args.boundary_edge_threshold,
                )
                loss += args.boundary_loss_coef * loss_boundary
                loss += args.boundary_footprint_loss_coef * loss_footprint
            psnr_ = psnr(image,gt_image).mean().double()       
            loss.backward()
    
            iter_end.record()
            ech_loss+=loss.item()
            boundary_loss_accum += loss_boundary.item()
            boundary_footprint_accum += loss_footprint.item()
            if (iteration)%len_timedata==0:
                ...
                if args.use_boundary_loss:
                    logging.info(
                        f"Iteration {iteration}: boundary_identity={boundary_loss_accum/len_timedata:.6f}, "
                        f"boundary_footprint={boundary_footprint_accum/len_timedata:.6f}"
                    )
            ...
            with torch.no_grad():
                ...
                if iteration % 10 == 0:            
                    progress_bar.set_postfix({"Loss": f"{ema_loss_for_log:.{7}f}",
                                              "psnr": f"{psnr_:.{2}f}",
                                              "b_id": f"{loss_boundary.item():.4f}",
                                              "b_fp": f"{loss_footprint.item():.4f}",
                                               "point":f"{total_point}"})
        ...
        if emissions_tracker is not None:
            try:
                ...
                logging.info(f"CodeCarbon tracking stopped. Emissions (kg CO2eq): {emissions_kg}")
            except Exception as exc:
                logging.warning(f"Failed to stop CodeCarbon tracker cleanly: {exc}")
